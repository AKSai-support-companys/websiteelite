...
/**
 * Non-cryptographic deterministic hash -> [0, 1)
 */
...
function resize() {
  ...
  const count = prefersReducedMotion ? Math.round(target * 0.7) : target;
...
function drawBackground() {
  ...
  const vignette = ctx.createRadialGradient(w * 0.5, h * 0.5, Math.min(w, h) * 0.2, w * 0.5, h * 0.5, Math.max(w, h) * 0.7);
...
function drawLinks(positions, t) {
  const maxD = linkDist;
  const maxD2 = maxD * maxD;

  const flow = prefersReducedMotion ? 0 : t * 0.02;
...
function drawNodes(positions) {
  ctx.save();
  ctx.shadowColor = "rgba(190, 230, 255, 0.42)";
  ctx.shadowBlur = 10;
...
function render(t) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    positions[i] = nodePosition(nodes[i], t);
  }
